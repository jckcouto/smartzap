...
import { EditConnectionOverlay } from "@/components/builder/overlays/edit-connection-overlay";
...
import { api, type Integration } from "@/lib/builder/api-client";
import {
  integrationsAtom,
...
const WHATSAPP_GLOBAL_INTEGRATION: Integration = {
  id: WHATSAPP_GLOBAL_INTEGRATION_ID,
...
export function IntegrationSelector({
  ...
  const loadIntegrations = useCallback(async () => {
    try {
      ...
      const withWhatsAppGlobal = all.some(
        (integration) => integration.id === WHATSAPP_GLOBAL_INTEGRATION_ID
      )
        ? all
        : [...all, WHATSAPP_GLOBAL_INTEGRATION];
      // Update global store so other components can access it
      setGlobalIntegrations(withWhatsAppGlobal);
      setHasFetched(true);
    } catch (error) {
