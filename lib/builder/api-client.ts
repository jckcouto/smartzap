...
export const aiApi = {
  generate: (
    ...
    apiCall<WorkflowData>("/api/builder/ai/generate", {
      method: "POST",
...
export const integrationApi = {
  ...
  getAll: (type?: IntegrationType) =>
    apiCall<Integration[]>(`/api/builder/integrations${type ? `?type=${type}` : ""}`),

  // Get single integration with config
  get: (id: string) =>
    apiCall<IntegrationWithConfig>(`/api/builder/integrations/${id}`),
  ...
  create: (data: {
    ...
    apiCall<Integration>("/api/builder/integrations", {
      method: "POST",
  ...
  // Delete integration
  delete: (id: string) =>
    apiCall<{ success: boolean }>(`/api/builder/integrations/${id}`, {
      method: "DELETE",
    }),
  ...
  testConnection: (integrationId: string) =>
    apiCall<{ status: "success" | "error"; message: string }>(
      `/api/builder/integrations/${integrationId}/test`,
      {
        method: "POST",
      }
    ),
...
export const userApi = {
  ...
  update: (data: { name?: string; email?: string }) =>
    apiCall<{ success: boolean }>("/api/builder/user", {
      method: "PATCH",
      body: JSON.stringify(data),
    }),
};
...
export const workflowApi = {
  // Get all workflows
  getAll: () => apiCall<SavedWorkflow[]>("/api/builder/workflows"),

  // Get a specific workflow
  getById: (id: string) =>
    apiCall<SavedWorkflow>(`/api/builder/workflows/${id}`),

  // Create a new workflow
  create: (workflow: Omit<WorkflowData, "id">) =>
    apiCall<SavedWorkflow>("/api/builder/workflows/create", {
      method: "POST",
  ...
  // Delete a workflow
  delete: (id: string) =>
    apiCall<{ success: boolean }>(`/api/builder/workflows/${id}`, {
      method: "DELETE",
    }),

  // Duplicate a workflow
  duplicate: (id: string) =>
    apiCall<SavedWorkflow>(`/api/builder/workflows/${id}/duplicate`, {
      method: "POST",
    }),

  // Get current workflow state
  getCurrent: () => apiCall<WorkflowData>("/api/builder/workflows/current"),

  // Save current workflow state
  saveCurrent: (nodes: WorkflowNode[], edges: WorkflowEdge[]) =>
    apiCall<WorkflowData>("/api/builder/workflows/current", {
      method: "POST",
  ...
  execute: (id: string, input: Record<string, unknown> = {}) =>
    apiCall<{
      ...
      error?: string;
      duration?: number;
    }>(`/api/builder/workflow/${id}/execute`, {
      method: "POST",
      body: JSON.stringify({ workflowId: id, input }),
    }),

  // Run workflow via builder executor
  run: (id: string) =>
    apiCall<{ executionId: string; status: string; workflowId: string }>(
      `/api/builder/workflows/${id}/run`,
      { method: "POST" }
    ),
  ...
  triggerWebhook: (id: string, input: Record<string, unknown> = {}) =>
    apiCall<{
      executionId: string;
      status: string;
    }>(`/api/builder/workflows/${id}/webhook`, {
      method: "POST",
  ...
  getExecutions: (id: string) =>
    ...
    >(`/api/builder/workflows/${id}/executions`),
  ...
  getExecutionLogs: (executionId: string) =>
    ...
    }>(`/api/builder/workflows/executions/${executionId}/logs`),
  ...
  getExecutionStatus: (executionId: string) =>
    ...
    }>(`/api/builder/workflows/executions/${executionId}/status`),
  ...
  download: (id: string) =>
    apiCall<{
      ...
      error?: string;
    }>(`/api/builder/workflows/${id}/download`),
...
export const aiGatewayApi = {
  ...
  getStatus: () => apiCall<AiGatewayStatusResponse>("/api/builder/ai-gateway/status"),

  // Get available Vercel teams
  getTeams: () => apiCall<AiGatewayTeamsResponse>("/api/builder/ai-gateway/teams"),

  // Grant consent and create managed API key
  consent: (teamId: string, teamName: string) =>
    apiCall<AiGatewayConsentResponse>("/api/builder/ai-gateway/consent", {
      method: "POST",
